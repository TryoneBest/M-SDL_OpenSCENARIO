...
class GeneralFunc:
    ...
    def cal_init(ego_speed, obj_speed_x, obj_x_range, obj_y_range):
        """
        计算目标车的初始状态，包括初始位置和初始速度
        :param ego_speed: 本车速度
        :param obj_speed_x: 目标车在场景时间里的速度列表
        :param obj_x_range: 目标车的纵向相对距离
        :param obj_y_range: 目标车的相对横向距离
        :return: 目标车初始纵向坐标，初始横向坐标，初始速度
        """
        left = 0
    ...
    def draw_filter(any_list, name, y_label):
        """
        滤波处理，画图并返回滤波后的数据
        :param any_list:要滤波的数据，一般是速度
        :param name:对象名，可不要，但懒得改了
        :param y_label:纵向的数据类型
        :return:list，滤波后的数据
        """
