...
class CutInScenario:
    ...
    def find_action(y_range_list):
        """
        找出事件的发生区间
        :param y_range_list: 与本车在场景运动时间内的横向相对距离
        :return: 元组，切入时间的起始时间，终止时间，切入方向（指目标车的变道方向）
        """
        i = 0
    ...
    def create_cut_in_scenario(self, gf, path, xodr_path, osgb_path, data_dic):
        """
        按照格式生成文件的主要函数
        :param gf: GeneralFuc类，包含场景公用的方法
        :param path: 输出路径
        :param xodr_path: 地图文件路径
        :param osgb_path: 地图数据库文件路径
        :param data_dic: parse_data函数处理过的vtti预读数据
        :return:
        """
        ...
        # 下面是按照OSC格式生成文件，具体参考format里的格式样例
        doc = Document()
        GF = kwargs['GF']
...
if __name__ == '__main__':
    ...
    out_path = 'cutin_example.xosc' # 输出路径
    xodr = '../Runtime/Tools/RodDistro_6710_Rod4.6.0/DefaultProject/Odr/city_quick_3.xodr' # odr地图文件路径
    osgb = '../Runtime/Tools/RodDistro_6710_Rod4.6.0/DefaultProject/Database/city_quick_3.opt.osgb' # osgb地图数据库文件路径
